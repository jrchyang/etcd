...
func (s *Snapshotter) save(snapshot *raftpb.Snapshot) error {
	...
	// 创建快照文件名，快照文件的名称由三部分组成，分别是快照所涵盖的最后一条
	// Entry 记录的 Term、Index 和 ".snap" 后缀
	...
	// 将序列化之后的数据和校验码封装成 snappb.Snapshot 实例。
	// 这里简单了解一下 raftpb.Snapshot 和 snappb.Snapshot 的区别，
	// 前者包含了 Snapshot 数据以及一些元数据（例如，该快照数据所涵盖的最后一条
	// Entry 记录的 Term 和 Index）；后者则是在前者序列化之后的封装，其中还记录了
	// 响应的校验码等信息
...
func (s *Snapshotter) loadMatching(matchFn func(*raftpb.Snapshot) bool) (*raftpb.Snapshot, error) {
	...
	for _, name := range names {
		// 按序读取快照文件，直到读取到第一个合法的快照文件，对于读取失败的快照文件
		// 则添加 ".broken" 后缀，这样在下次调用 snapNames() 方法获取可用
		// 快照文件时，就会将其过滤掉
...
func loadSnap(lg *zap.Logger, dir, name string) (*raftpb.Snapshot, error) {
	...
	if err != nil {                   // 如果读取失败，则将该快照文件的后缀名修改成 ".broken"
