...
func (ro *readOnly) addRequest(index uint64, m pb.Message) {
	// 在 ReadIndex 消息的第一个记录中，记录了消息 ID
...
func (ro *readOnly) advance(m pb.Message) []*readIndexStatus {
	...
	// 查找到指定的 MsgReadIndex 消息 ID，则清空 readOnly 中所有在它之前的消息 ID
	// 及相关内容
	if found {
		// 清理 readOnly.readIndexQueue
		ro.readIndexQueue = ro.readIndexQueue[i:]
		// 清理 readOnly.pendingReadIndex
		for _, rs := range rss {
